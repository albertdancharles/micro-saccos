...
export default function LoanQueueItem({ loan, onActioned }) {
  ...
  const overLimit = Number(loan.principal) > Number(loan.ceiling)
  ...
  return (
    <div className="rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="rounded-lg bg-gray-50 p-3 text-sm space-y-1">
        <Row label="Approved savings" value={formatTZS(loan.savings)} />
        <Row label="5× ceiling" value={formatTZS(loan.ceiling)} />
        <Row label="Requested" value={formatTZS(loan.principal)} danger={overLimit} />
        {overLimit && <p className="text-xs text-red-600">Exceeds the member's 5× savings limit.</p>}
      </div>
