    ...
    <div className="space-y-3">
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-sky-50 to-white p-4 sm:p-5 ring-1 ring-sky-200/70 shadow-[0_1px_2px_-1px_rgba(15,23,42,0.04),0_8px_24px_-16px_rgba(15,23,42,0.10)]">
        ...
        <p className="mt-1.5 text-xs text-sky-700/75">
          {formatTZS(stats.pool)} {t('in the pool')} · {formatTZS(outstanding)} {t('out on loans')}
        </p>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatCard label={t('Group pool')} value={formatTZS(stats.pool)} />
        <StatCard label={t('Fees this month')} value={`${stats.feesPaid} / ${stats.feesTotal}`} />
        <StatCard label={t('Active loans')} value={stats.activeLoans} />
        <StatCard
